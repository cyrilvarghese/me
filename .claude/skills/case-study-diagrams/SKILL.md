...
**The traveller** is a round dot — a near-zero round-capped dash, whose
painted mark is a circle of stroke-width diameter:
...
`stroke-width = 11 × (viewBox width / 548)`. In HTML, an 11px circle.
One variant exists: 02-canvas runs a long border with a 9.6 dash at
width 3.4 — a comet for tracing an outline, not the marker for riding
a rail.
...
| Traveller | red dot, 11 across (near-zero dash, round caps, width 11); the 02-canvas border-run uses a 9.6×3.4 comet variant |
...
3. Traveller at 246 units/second — the 11-wide dot, constant painted size?
